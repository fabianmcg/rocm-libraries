...
#
# Integration test for the tensilelite-client --use-rstd-scale path with bfp8 (OCP e5m2) A/B input.
#
# Steps:
...
#
# Usage:
#   epilogues/scripts/test_client_rstdscale_bf8.sh [--chip CHIP] [--client PATH] [--out-dir DIR]
#
# Arguments:
#   --chip    GPU architecture string (default: gfx950).
#   --client  Path to the tensilelite-client binary (required if not on PATH).
#   --out-dir Scratch directory for generated files (default: /tmp/rstd_bf8_test_<chip>).
#
# Prerequisites:
...
if [[ -z "$OUT_DIR" ]]; then
    OUT_DIR="/tmp/rstd_bf8_test_${CHIP}"
fi
...
python3 "${TENSILELITE_ROOT}/Tensile/bin/Tensile" \
    "${TENSILELITE_ROOT}/epilogues/yaml/gemm_rstdscale_k3_bf8.yaml" \
    "${OUT_DIR}/tensile_out"
...
# A/B are OCP bfp8 e5m2; C/D remain bf16 (RstdScale output is bf16).
