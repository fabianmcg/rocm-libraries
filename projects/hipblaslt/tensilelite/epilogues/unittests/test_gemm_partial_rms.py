...
def _run_shape(solution, kernelName, hsaco, chip, M, K, nHidden, residualAdd=False):
    ...
    h1Eff = h1 + residualEff if residualAdd else h1
    dRef  = (h1Eff * gammaEff[np.newaxis, :]).astype(ml_dtypes.bfloat16)
...
# ---------------------------------------------------------------------------
# Test: DP-only PartialRMS with f32 (and bf16) Gamma/Residual types.
# ---------------------------------------------------------------------------

_K1_TYPES_SOLUTIONS = enumerateSolutions("gemm_partial_rms_k1_types.yaml")


@pytest.fixture(
    scope="session",
    params=[sol for sol, _id in _K1_TYPES_SOLUTIONS],
    ids=[sid for _sol, sid in _K1_TYPES_SOLUTIONS],
)
def k1_types_kernel(request):
    """Assemble and compile one DP-only K1 PartialRMS solution with configured side types."""
    solution = request.param
    kernelName, hsaco, chip = compileSolution(solution)
    residualAdd = bool(solution.get("PartialRMSResidualAdd", False))
    return solution, kernelName, hsaco, chip, residualAdd
...
@pytest.mark.parametrize(
    "M,K,N_hidden,note",
    _K1_TYPES_SHAPES,
    ids=[note for *_, note in _K1_TYPES_SHAPES],
)
def test_k1_dp_types(k1_types_kernel, M, K, N_hidden, note):
    """Verify DP-only K1 D and partialBuf with f32/bf16 Gamma+Residual."""
    solution, kernelName, hsaco, chip, residualAdd = k1_types_kernel
    MT0 = solution["MacroTile0"]
    nD  = math.ceil(N_hidden / MT0)
    dGpu, dRef, pbGpu, sumsqRef = _run_shape(
        solution, kernelName, hsaco, chip, M, K, N_hidden, residualAdd
    )
    assertClose(dGpu, dRef, note, kind="D")
    assertClose(pbGpu[:M, :], sumsqRef, f"{note} n_d={nD}",
                rtol=1e-4, atol=1e-4, kind="partialBuf")
