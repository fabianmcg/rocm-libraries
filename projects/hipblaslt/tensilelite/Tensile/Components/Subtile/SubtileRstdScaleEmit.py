...
class SubtileRstdScaleEmitter:
    ...
    def _loadRstd(self, rstd_srd: int, row_base_sgpr: int, row_group_byte: int,
        ...
        offset_tmp  = self.writer.vgprPool.checkOut(1, tag="rstdTmp")
            ...
            for k in range(self.rows_per_lane):
                ...
                if m_k_offset > 64:
                    module.add(VMovB32(dst=vgpr(offset_tmp), src=m_k_offset,
                                       comment=f"offset_tmp = {m_k_offset} (literal too large for VAddU32)"))
    ...
    def _applyScale(self, vgprTiles, rstd_vgprs: int) -> Module:
    ...
    def emit(self, vgprTiles) -> Module:
        ...
        vgprTiles: dtileInfo.vgprTiles, the per-tile allocator records for the D accumulator.
        """
        ...
        module.add(self._applyScale(vgprTiles, rstd_vgprs))
