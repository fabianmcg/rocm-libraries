...
class SubtilePartialRMSEmitter:
    ...
    def _setup(
        ...
        module.add(SMovB64(dst=sgpr(partial_srd, 2), src=sgpr("PartialBuf", 2),
                           comment="partialBuf SRD base"))
        module.add(SMovB32(dst=sgpr(partial_srd + 2), src="BufferOOB",
        ...
        module.add(VAndB32(dst=vgpr(col_byte), src0=vgpr(lane_id), src1=self.mfma_n - 1,
        ...
        if self.wg_n > 1:
            ...
            module.add(vectorStaticDivide(
                wave_n, "Serial", self.wave_size * self.wg_m, tmp_res,
            ...
            with self.writer.allocTmpSgpr(1, tag="pRMS_setupColBase") as tmpSgprInfo:
                ...
                module.add(VMulLOU32(dst=vgpr(wave_n), src0=sgpr(tmpSgprInfo.idx),
                                     src1=vgpr(wave_n),
    ...
    def _crossWaveReduce(self, partials: int) -> Module:
        ...
        group_stride = self.wg_m * stride_w                 # bytes between N siblings
        ...
        # Ensure sibling waves are done with main-loop LDS before overwriting.
        ...
        # wg_m must be power-of-two (enforced by _validatePartialRMS).
        module.add(VAndB32(dst=vgpr(wave_m), src0=vgpr(wave_id), src1=self.wg_m - 1,
                           comment=f"waveM = waveId %% {self.wg_m} (bitmask, wg_m pow2)"))

        with self.writer.allocTmpSgpr(1, tag="pRMS_xwSgpr") as tmpSgprInfo:
    ...
    def _writePartials(
        ...
        module.add(VAndB32(dst=vgpr(col_in_mma), src0=vgpr(lane_id), src1=self.mfma_n - 1,
                           comment="col_in_mma = lane_id % mfma_n"))
        module.add(VCmpEQU32(
