...
class SubtilePartialRMSEmitter:
    ...
    def emit(self, vgprTiles) -> Module:
        ...
        amaxArg = self.amaxPartials if self.quant else None
        module.add(self._fusedAccPassFree0(
            vgprTiles, self.gammaSrd, self.partials, amaxArg, absMask, self.globalAddr))
        if self.quant:
    ...
    def _fusedAccElement(self, module, vgprTiles, accReg: int, gammaReg: int, resReg, partials: int,
    ...
    def _fusedAccRow(self, module, vgprTiles, accBurst: int, gammaBurst: int, resBurst,
                     partials: int, amaxPartials, absMask, m: int) -> None:
        for k in range(self.rows_per_lane):
            ...
            for n in range(self.mma_n):
                ...
                self._fusedAccElement(module, vgprTiles, accBurst + n, gammaBurst + k, resReg,
                                      partials, amaxPartials, absMask, m, n, k, first)

    def _fusedAccPassFree0(self, vgprTiles, gammaSrd: int, partials: int,
        ...
        for m in range(self.mma_m):
            ...
            waitLabel = "gamma+residual" if resBurst is not None else "gamma"
            module.add(SWaitCnt(vlcnt=0, comment=f"wait {waitLabel} row burst."))
