...
def _formatGflopsTable(rows):
    ...
    for feature, size, pyGf, cppGf in rows:
        ...
        cppStr = f"{cppGf:.1f}" if cppGf is not None else "N/A"
...
def _buildReportContent(data) -> str:
    """Build the parity report markdown string from collected test data."""
    ...
    if data["discrepancies"]:
        discLines = "\n".join(f"- **{f}**: {msg}" for f, msg in data["discrepancies"])
    else:
        discLines = "_None recorded within stated tolerances._"
    note = (
    ...
    return (
        ...
        f"## Feature Coverage\n\n{featureTable}\n\n"
        f"## GFLOPS Comparison (Python vs C++ client)\n\n{gflopsTable}\n\n"
        f"{note}\n\n"
        f"## Remaining Discrepancies\n\n{discLines}\n"
    )


def _writeParityReport(data, reportPath):
    """Write parity_report.md from collected test data."""
    content = _buildReportContent(data)
