...
| `HIPBLASLT_FUSED_EPILOGUE_REQUANT_BLOCK_SIZE`         | `int32_t`                               | MX tile column extent (`tile_cols`, elements per tile along N) for `MX_BLOCK` granularity; power-of-two >= 1 (default 32)          |
| `HIPBLASLT_FUSED_EPILOGUE_REQUANT_TILE_ROWS`          | `int32_t`                               | MX tile row extent (`tile_rows`, rows per tile) for `MX_BLOCK` granularity; power-of-two >= 1, default 1 = per-row blocks          |
...
**Tile layout:** one UE8M0 scale per 2D tile of shape `[tile_rows, tile_cols]`, stored row-major
over the tile grid as `[ceil(M/tile_rows), ceil(N/tile_cols)]`. `tile_cols` partitions the output
N dimension (which equals K1 of GEMM2, the contraction dimension); `tile_rows` partitions the M
dimension. Both dimensions are parametric: `tile_cols` via
`HIPBLASLT_FUSED_EPILOGUE_REQUANT_BLOCK_SIZE` (power-of-two >= 1, default 32) and `tile_rows` via
`HIPBLASLT_FUSED_EPILOGUE_REQUANT_TILE_ROWS` (power-of-two >= 1, default 1 = per-row blocks, the
existing behaviour). Quantized output is OCP E4M3 FP8 with `mxfp8_max = 448.0`.

**API attributes:**

- `SCALE_GRANULARITY = HIPBLASLT_REQUANT_SCALE_MX_BLOCK` selects MX tile mode.
- `SCALE_COMPUTE_MODE = DYNAMIC_FROM_AMAX` is required; static scales are not supported for this
  mode.
- `SCALE_POINTER` points to the `uint8_t` UE8M0 scale array of shape
  `[ceil(M/tile_rows), ceil(N/tile_cols)]`, row-major over the tile grid.
- `HIPBLASLT_FUSED_EPILOGUE_REQUANT_BLOCK_SIZE` sets `tile_cols` (column extent of each tile).
- `HIPBLASLT_FUSED_EPILOGUE_REQUANT_TILE_ROWS` sets `tile_rows` (row extent of each tile,
  default 1).
